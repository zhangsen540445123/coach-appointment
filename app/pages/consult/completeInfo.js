            ...
            (function(e, i) {
                ...
                var V = {
                    ...
                    onLoad: function(e) {
                        ...
                        if (e.visitorID && "undefined" !== e.visitorID) {
                            // 传递了 visitorID，使用 ID 查询
                            t.latestVisitorDetail(e);
                        } else {
                            // 没有传递 visitorID，尝试根据 userId 查询已有的客户信息
                            t.loadVisitorInfoByUserId();
                        }
                        this.params.orderId && this.cancelTimec.start([this.params]);
                        setTimeout((function() {
                    ...
                    methods: {
                        ...
                        // 根据 userId 加载客户信息（当没有传递 visitorID 时使用）
                        loadVisitorInfoByUserId: function() {
                            var t = this,
                                o = this;
                            // 等待 globalData 加载完成后查询
                            setTimeout(function() {
                                var globalData = (0, m.storeGet)("counselor_info");
                                if (globalData && globalData.userId) {
                                    (0, u.irequestdata)({
                                        url: "/visitor/visitorInfo/showUserVisitorInfoByUserid",
                                        method: "get",
                                        data: { userId: globalData.userId },
                                        success: function(e) {
                                            if (200 === e.data.code && e.data.data && e.data.data.length > 0) {
                                                // 获取第一条客户信息记录
                                                var visitorInfo = e.data.data[0];
                                                o.formVisitor = Object.assign({}, o.formVisitor, visitorInfo);
                                                o.formVisitor.userId = globalData.userId;
                                                if (visitorInfo.mobileArea) {
                                                    o.needToRealName.mobileArea = visitorInfo.mobileArea;
                                                }
                                                if (visitorInfo.name) {
                                                    t.isRealName = true;
                                                }
                                                // 处理图片列表
                                                if (visitorInfo.otherHistoryUrl && visitorInfo.otherHistoryUrl.length > 0) {
                                                    o.countImageList = [];
                                                    for (var i = 0; i < visitorInfo.otherHistoryUrl.length; i++) {
                                                        o.countImageList.push({
                                                            url: visitorInfo.otherHistoryUrl[i],
                                                            fileName: visitorInfo.otherHistoryUrl[i]
                                                        });
                                                    }
                                                    o.tempImageList = o.countImageList;
                                                }
                                                console.log("已加载客户信息:", o.formVisitor);
                                            }
                                        },
                                        error: function() {
                                            console.log("加载客户信息失败");
                                        }
                                    });
                                }
                            }, 600);
